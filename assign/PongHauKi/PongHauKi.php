...
function startTimer(){
    ...
    timer = setInterval(function(){
        ...
        if(sec <= 0){
            ...
            //     finishFlag = 2;
                if(mode == 2){
                    alert("Your time out, Your competitor wins!");
                }else{
...
//1:READY TO START 0:RUNING 2:STOP
...
//            setInterval(getGameStatus,500);
        ...
        if(mode == 2){
            ...
            },4,{'turn':1,'status':status,'position':position});
        }
...
<?}?> 
    ...
    stop();
    statusIntervalId = setInterval(getGameStatus,1000);
    ...
    if(finishFlag == -1 && neighbours[sourcePos].indexOf(targetPos)!=-1 && posStatus[targetPos]==0){
        ...
        if(finishFlag == 1){
            if(mode == 1){
                ...
                alert("Game Over. "+'<? if(isset($_SESSION['playerNum'])&&$_SESSION['playerNum']==2&&isset($_SESSION['competitor'])){echo ($_SESSION['competitor']['name'].'wins');}else{echo 'You win';}?>');
            }
            stop();
        }else if(finishFlag == 2){
            alert("Game Over."+'<?php if(isset($_SESSION['playerNum'])&&$_SESSION['playerNum']==1&&isset($_SESSION['competitor'])){echo ($_SESSION['competitor']['name'].'wins');}else{echo 'You win';}?>');
            stop();
        }else{
...
<script>
    ...
    <?php if(isset($_SESSION['username'])&&isset($_SESSION['lat'])){?>
   ...
   function getGameStatus(){
            ...
            send(function (data) {
		            if(data.success == 1){
			            ...
			            alert("Competitor exits");
			            // alert(data.ss);
			            clearInterval(statusIntervalId);
		       		}
		       	},5,{"status":status,'mode':mode});
